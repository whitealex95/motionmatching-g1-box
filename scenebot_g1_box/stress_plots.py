"""Plots for the box stress test report (reads out/stress/results.json)."""
import json
import os
...
STATUS = {'SUCCESS': ('#0ca30c', '✓'),   # good, check
          'DROPPED': ('#fab219', '▽'),   # warning, open triangle
          'NO GRIP': ('#ec835a', '○'),   # serious, open circle
          'FELL': ('#d03b3b', '✕'),      # critical, cross
          'PARTIAL': ('#ec835a', '◇')}   # serious, diamond
...
def main():
    ...
    # shape sweeps: grip width / height / depth / other shapes
    ...
    panels = [('width', 'grip width 2·hy (m)',
              ...
              ('shape', 'cylinder / sphere',
               lambda r: r['name'].replace('_', '\n'))]
    for ax, (g, xlabel, fmt) in zip(axes, panels):
        rg = group(rows, g)
        used.update(r['outcome'] for r in rg)
        if g == 'shape':
            th = [(r['box_size'][2] if r['box_type'] == 'cylinder'
                   else r['box_size'][0]) + 0.25 for r in rg]
        elif g == 'height':
            th = [r['box_size'][2] + 0.25 for r in rg]
        else:
            th = [0.15 + 0.25] * len(rg)
    ...
    rg = group(rows, 'grid') + [r for r in rows
                                if r['name'] in ('scale_0.8', 'scale_1',
                                                 'scale_1.2', 'mass_0.5kg',
                                                 'mass_2kg', 'mass_5kg')]
    scales = [0.8, 1.0, 1.2]
    masses = [0.1, 0.5, 2.0, 5.0]
    ...
    for i, s in enumerate(scales):
        for j, m in enumerate(masses):
            match = [r for r in rows
                     ...
                     and r['box_mass'] == m and r['box_type'] == 'box']
            if not match:
                continue
            o = match[0]['outcome']
            col, glyph = STATUS[o]
    ...
    lines = ['| run | type | size (m, full) | mass | outcome | max box z |',
             '|---|---|---|---|---|---|']
    for r in rows:
        if 'dedup_of' in r:
            continue
        sz = ' × '.join(f'{2 * v:g}' for v in r['box_size'][:3]
                             if v > 0)
        lines.append(f"| {r['name']} | {r['box_type']} | {sz} | "
                     f"{r['box_mass']:g} kg | {r['outcome']} | "
                     f"{r['max_box_z']:.2f} |")
